...
struct TArrayArchive : public SArchive
{
	...
	{
		const SizeType offsetInElements = offset >= ELEMENT_SIZE ? SMathOps::Floor(offset / ELEMENT_SIZE) : 0;
	...
	virtual SizeType ReadBytes(void* ptr, SizeType size) override
	{
		...
		const SizeType elementsToRead = SMathOps::Min<SizeType>(
			SMathOps::Floor(size / ELEMENT_SIZE),
	...
	virtual SizeType WriteBytes(const void* ptr, SizeType size) override
	{
		...
		const SizeType elementsToWrite = SMathOps::Floor(size / ELEMENT_SIZE);
