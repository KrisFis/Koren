...
#include "Kor/Memory/Detail/SharedReferencer.h"
#include "Kor/Memory/Detail/SharedTypeTraits.h"
...
private:
	...
	mutable Detail::SReferencerProxy _referencerProxy = nullptr;
};
...
KOR_FORCEINLINE_DEBUG static TSharedPtr<T> MakeShareable(InstanceT* instance)
{
	...
	Detail::CReferencerBase* referencer = Detail::NewCustomReferencer(instance);
	TSharedPtr<T> newShared = TSharedPtr<T>(*referencer);
	if constexpr (Detail::TIsSharedClassType<T>::Value)
	{
