...
struct SLinuxPlatformCString
{
	...
	// Converts integer value to string in specified base, string is saved to provided buffer
	FORCEINLINE static char* FromInt64(int64 val, char* buf, uint32 maxLen) { snprintf(buf, maxLen, "%ld", val); return buf; }
	FORCEINLINE static wchar* FromInt64(int64 val, wchar* buf, uint32 maxLen) { swprintf(buf, maxLen, L"%ld", val); return buf; }
